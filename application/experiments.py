    ...
    for path in img_paths:
        ...
        input_list = input_list + \
            list(itertools.product(
                [path], sigma_s_list, sigma_r_list, alpha_r_list))
...
def apply_methods(row, input_img, tol, channels, rho):
    ...
    output_img_farbman, outs_farbman = wlss.admm_method_farbman(input_img,  row['sigma_s'], row['sigma_r'], row['alpha_r'],
                                                                tol=tol, rho=rho, channels=channels)
    img_psnr = PSNR(output_img_farbman[:, :, 0],
                    output_img_gastal[:, :, 0], data_range=255)
    img_ssim = SSIM(output_img_farbman[:, :, 0],
                    output_img_gastal[:, :, 0], data_range=255)
...
def get_remaining_inputs(img_paths):
    ...
    df_existing_inputs = pd.read_csv("outputs.csv")[
        ['image', 'sigma_s', 'sigma_r', 'alpha_r']]
    ...
    for img_name, group in df_inputs.groupby('image'):
        ...
        for index, row in df_image.iterrows():
            ...
            if dt_average == 0:
                ...
                # SES filter in dt to estimate 'average' dt
                dt_average = 0.5*dt_average + 0.5*dt
            ...
            if t % 10 == 0:
                ...
                if (t % 100 == 0) or (t == len(df_image)):
                    ...
                    result_list = []
                    print("Parsed image {}, {}/{}, estimated remaining time = {} days".format(
                        row['image'], t, len(df_image), round((len(df_image)-t)*dt_days, 2)))
                ...
                result_list = []
                print("Parsed image {}, {}/{}, estimated remaining time = {} days".format(
                    row['image'], t, len(df_image), round((len(df_image)-t)*0, 2)))
