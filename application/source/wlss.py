...
def get_wx_wy(luminance,  lambda_val, alpha, epsilon, tol=1e-3, mode='truncate'):
...
def solve_hermitian_system(A, g, tol=1e-10, max_iter=1000):
    ...
    f, info = cg(A, np.matrix.flatten(g), rtol=tol,
                 maxiter=max_iter, x0=np.matrix.flatten(g))
...
def admm_method_farbman(input_img, sigma_s, sigma_r, alpha_r, tol=1e-3, rho=10, bound=0, channels=1, mode='truncate'):
    # apply conversion
    ...
    Wx, Wy = get_wx_wy(luminance, lambda_val, alpha, epsilon, mode=mode)
    max_it = 10000
    ...
    for c in range(channels):
        ...
        while (err > tol) and (it < max_it):
            X = dtf.tikhonov1D_x(
                (mono_image + rho*Y - U)/(1+rho), 2*Wx/(1+rho), mode=mode)
            Y = dtf.tikhonov1D_y(
                (mono_image + rho*X + U)/(1+rho), 2*Wy/(1+rho), mode=mode)
            U = U + rho*(X-Y)
            it += 1
